...
![NBK Banking Avatar in Action](voicelive-api-salescoach/docs/assets/preview.png)
...
![Banking Customer Service Dashboard](voicelive-api-salescoach/docs/assets/analysis.png)
...
└─ voicelive-api-salescoach/
   ├─ backend/          # Python Flask API + WebSocket bridge
   ├─ frontend/         # React + Fluent UI client
...
2. **Configure Bing Custom Search**:
   ...
   - Add NBK website domains (e.g., `https://www.nbk.com`, `https://www.nbk.com/kuwait`)
...
> ℹ️  Any value you keep in `voicelive-api-salescoach/.env` for local runs (Azure AI endpoints, Speech keys, etc.) should also be stored in the Azure Developer CLI environment using `azd env set` / `azd env set --secret`. See the [Deployment Guide](voicelive-api-salescoach/DEPLOYMENT-GUIDE.md#5-supply-service-endpoints-for-cloud-runtime) for the exact commands.

### Local Development

The project lives under `voicelive-api-salescoach/` and includes a dev container for easy setup plus helper scripts.
...
2. **Fill in environment variables**
   - Copy `voicelive-api-salescoach/.env.template` to `voicelive-api-salescoach/.env`
...
<tr>
<td width="400">
<img src="voicelive-api-salescoach/docs/assets/architecture.png" alt="Architecture Diagram" width="500"/>
</td>
<td>
...
If you believe you have found a security vulnerability in any Microsoft-owned repository that meets [Microsoft's definition of a security vulnerability](https://aka.ms/security.md/definition), please report it to us as described in [SECURITY.md](voicelive-api-salescoach/SECURITY.md).
